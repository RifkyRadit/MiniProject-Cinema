...
//        print ("QUERY INSERT SCHEDULE: \(queryString)")
    ...
    //check schedule
    func checkSchedule(check: [String: String]) -> Bool{
        var stmt : OpaquePointer?
        
        let idFilm = check["idFilm"]!
        let idTheater = check["idTheater"]!
        let hour = check["hours"]!
        
        let queryString = "select Schedule.* from Schedule WHERE idFilm='\(idFilm)' and idTheater='\(idTheater)' and hours='\(hour)'"
